...
enum MoveResult {moved,impCollision,impEnter,noEngine,outOfEnergy,badDestination,error}
...
class MovementController extends FugueController {
  ...
  MoveResult moveShip(Ship? ship, Coord3D c, {double baseEnergy = 20}) {
    ...
    if (ship.loc.domain == Domain.impulse && ship.loc.level.shipsAt(destination).isNotEmpty) {
    ...
    if (engine == null) {
      if (ship.playship) fm.msgController.addMsg("Error: no engine");
